...
velocity = 80
tempo = bpm2tempo(bpm)
octave_shifts = [0, 0, 0, 0, 0, 0, 0 ]  # Octave shifts per channel
base_grid = 100  # 100ms per grid unit (81 = 100ms)
# === Note Range Limitation ===
MIN_NOTE = 24
MAX_NOTE = 119
def clamp_note(note):
    """Ensure note is within valid MIDI range (24-119)"""
...
# Unknown command (73) - 1 parameter
        ...
        elif byte in range(0x0, 0x60) and i+1 < len(data) and data[i+1] >= 0x81:
            duration = get_grid_multiplier(data[i+1]) * base_grid
